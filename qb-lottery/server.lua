...
local Config = {
	...
	RandomWinChance = 10,   -- 机选必中当前开奖号码概率（100=必中）
...
RegisterNetEvent('qb-lottery:server:Buy', function(num, type)
    ...
    if not canBuy then
        TriggerClientEvent('QBCore:Notify', src, "最后10秒禁止购买", "error")
        return
    end
    ...
    if not num:match("^%d%d%d%d$") then
        TriggerClientEvent('QBCore:Notify', src, "必须4位数字", "error")
        return
    end

    playerTickets[src] = playerTickets[src] or {}
    if #playerTickets[src] >= Config.MaxTickets then
        TriggerClientEvent('QBCore:Notify', src, "最多5注", "error")
        return
    end

    local cash = Player.PlayerData.money["cash"] or 0
    if cash < Config.Price then
        TriggerClientEvent('QBCore:Notify', src, "现金不足", "error")
        return
    end
    ...
    TriggerClientEvent('QBCore:Notify', src, "购买成功："..num, "success")
end)
...
RegisterNetEvent('qb-lottery:server:Random', function()
    ...
    TriggerClientEvent('QBCore:Notify', src, "机选成功："..randomNum, "success")
end)
    ...
    while true do
        ...
        if #winnerList > 0 then
            ...
            for _, w in pairs(winnerList) do
                local p = QBCore.Functions.GetPlayer(w.id)
                ...
                if p then
                    ...
                    p.Functions.AddMoney("cash", reward)
